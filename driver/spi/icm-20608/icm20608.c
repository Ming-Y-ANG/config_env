...
static int icm20608_read_regs(struct icm20608_dev *dev, unsigned char reg, void *buf, int len)
{
	int ret = -1;
...
static int icm20608_write_regs(struct icm20608_dev *dev, unsigned char reg, void *buf, int len)
{
	int ret = -1;
	struct spi_message msg;
	struct spi_transfer xfer = {0};
...
{
	return icm20608_read_regs(dev, reg, buf, 1);
}

static int icm20608_write_reg(struct icm20608_dev *dev, unsigned char reg,  unsigned char data)
{
